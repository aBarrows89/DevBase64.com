// Run this script with: node scripts/runImport.mjs
import { ConvexHttpClient } from "convex/browser";

// Get the Convex URL from environment or use production URL
const CONVEX_URL = process.env.CONVEX_URL || "https://outstanding-dalmatian-787.convex.cloud";

const client = new ConvexHttpClient(CONVEX_URL);

// Employee data extracted from PDF (61 employees)
const employees = [
...
async function runImport() {
  ...
  try {
    const result = await client.mutation("personnel:bulkImport", { employees });
    console.log("\n=== Import Results ===");
    console.log(`Imported: ${result.imported}`);
    console.log(`Skipped (duplicates): ${result.skipped}`);
    if (result.skippedEmails.length > 0) {
      console.log(`Skipped emails: ${result.skippedEmails.join(", ")}`);
    }
  } catch (error) {
